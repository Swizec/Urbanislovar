...
{
	name = dd.obj.name;
	dd.obj.prevx = dd.obj.x;
	dd.obj.prevy = dd.obj.y;
	
	shadow = name.replace( /file/, 'shadow' );
	icon = name.replace( /file/, 'icon' );
	replacement = name.replace( /file/, 'replacement' );
	
	dd.elements[ shadow ].show();
	...
	//getStyleObject( replacement ).display = 'block';
	dd.obj.detachChild( dd.elements[ replacement ] );
	dd.elements[ replacement ].show();
...
function my_DropFunc()
{
	...
	//topId = getDirTop( dd.obj.name );
	//dd.obj.setZ( dd.elements[ 'file_0_'+topId ].z );
	//getStyleObject( shadow ).display = 'none';
	
	x = dd.obj.x;
	y = dd.obj.y;
	xx = dd.obj.x+dd.obj.w;
	yy = dd.obj.y+dd.obj.h;
	
	image = dd.elements[ 'image' ];
	x1 = image.x;
	y1 = image.y;
	xx1 = image.x+image.w;
	yy1 = image.y+image.h;
	
	id = dragOverDirF( dd.obj );
	
	if ( id != -1 )
	{
		moveFile( id, name.replace( /file_/, '' ) );
	}else if ( ( x >= x1 && y >= y1 && x <= xx1 && y <= yy1 ) ||
		( xx >= x1 && yy >= y1 && xx <= xx1 && yy <= yy1 ) )
	{
		id = name.replace( /file_/, '' );
		changeDisplay( id );
	...
	{
		refreshFile( name.replace( /file_/, '' ) );
	}
}